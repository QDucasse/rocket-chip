...
object Instructions {
  // RIMI: RIMI instructions and their opcodes
  ...
  def LS                 = BitPat("b?????????????????011?????0111011")
  def SS                 = BitPat("b?????????????????011?????0001111")
...
object CSRs {
  ...
  val all = {
    ...
    res += pmpaddr62
    res += pmpaddr63
    res += dmpcfg     // RIMI: dmpcfg addition to the csr list
    res += curdom     // RIMI: curdom addition to the csr list
    res += mseccfg
    res += tselect
    res += tdata1
    res += tdata2
